...
from utils.file_sync import handle_folder_rename
import logging
...
def create_locomotive_head_from_form(form_data, is_json=False):
  ...
  return LocomotiveHead(
    ...
    light_model_id=safe_int(get_value('light_model_id')),
...
def update_locomotive_head_from_form(locomotive_head, form_data):
  ...
  locomotive_head.light_model_id = safe_int(form_data.get('light_model_id'))
...
def api_edit_locomotive_head(id):
  ...
  try:
    ...
    # 品牌或货号变化时重命名文件夹与文件记录（用品牌缩写）
    new_brand = db.session.get(Brand, locomotive_head.brand_id)
    handle_folder_rename('locomotive_head', id, old_brand, old_item_number,
                         new_brand, locomotive_head.item_number or '')
...
def delete_locomotive_head(id):
  ...
  try:
    ...
    delete_model_files('locomotive_head', id)
