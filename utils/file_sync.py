...
def sanitize_path_segment(segment):
  """净化路径段：去除路径分隔符与控制字符，保留中文等普通字符。

  Args:
    segment: 原始路径段

  Returns:
    str: 净化后的路径段；空或全非法时返回 '_'
  """
  if not segment:
    return '_'
  cleaned = re.sub(r'[\x00-\x1f]', '', str(segment))
...
def safe_join_within(base_dir, *parts):
  """安全拼接路径并校验结果仍在 base_dir 内，防止路径遍历。

  先对各段做 sanitize_path_segment，再用 realpath 校验最终路径未越出 base_dir。
  ...
  Raises:
    ValueError: 拼接结果越出 base_dir
  """
  sanitized = [sanitize_path_segment(p) for p in parts if p]
  if not sanitized:
    raise ValueError('无有效路径段')
...
def get_absolute_file_path(file_path):
  """将相对 file_path 解析为绝对路径，校验仍在 DATA_DIR 内。

  file_path 是相对 DATA_DIR 的完整路径（含子目录分隔符，各段在上传时已净化）。
  这里不重新净化（避免破坏合法的目录分隔符），仅用 realpath 做边界校验防越界。

  Args:
    file_path: 相对 DATA_DIR 的路径（通常来自 ModelFile.file_path）

  Returns:
    str: 校验通过的绝对路径

  Raises:
    ValueError: 路径越出 DATA_DIR
  """
  if not file_path:
...
def get_model_folder_path(model_type: str, brand_abbreviation: str, item_number: str) -> str:
  """
  获取模型文件存储路径（净化各段并校验未越出 DATA_DIR）
  ...
  return safe_join_within(data_dir, model_type, folder_name)
...
def handle_folder_rename(model_type, model_id, old_brand, old_item_number, new_brand, new_item_number):
  """编辑模型后，若品牌缩写或货号变化，重命名文件夹与文件记录。

  统一 4 个模型路由 edit 的重命名逻辑。使用品牌缩写（与文件命名规则一致）。

  Args:
    model_type: 模型类型
    model_id: 模型 ID
    old_brand: 旧 Brand 对象（None 表示无）
    old_item_number: 旧货号
    new_brand: 新 Brand 对象
    new_item_number: 新货号
  """
  old_abbr = old_brand.abbreviation if old_brand else ''
  new_abbr = new_brand.abbreviation if new_brand else ''
  if old_abbr == new_abbr and old_item_number == new_item_number:
    return
  rename_model_folder(model_type, old_abbr, old_item_number, new_abbr, new_item_number)
  update_file_records_in_db(model_type, model_id, old_abbr, old_item_number,
                            new_abbr, new_item_number)
