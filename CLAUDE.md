...
| brand | 品牌 - 所有模型共享，包含 abbreviation（缩写）字段用于文件命名|
...
### 第七次优化内容（v0.9.0）
1. **品牌缩写功能**
   - Brand 模型新增 `abbreviation` 字段（必填、唯一、最长 10 字符）
   - 信息维护页面支持品牌缩写的增删改查
   - 新增品牌时自动生成拼音首字母缩写（可手动修改）

2. **文件命名优化**
   - 文件目录和命名改用品牌缩写替代品牌全名
   - 目录结构：`模型类型/品牌缩写_货号/`
   - 文件命名：`品牌缩写_货号.{ext}`（图片）、`品牌缩写_货号_FunctionKey.{ext}`（功能表）

3. **新增依赖**
   - pypinyin 库用于中文品牌名生成拼音首字母缩写

4. **工具函数**
   - `generate_brand_abbreviation()`: 根据品牌名生成拼音首字母缩写
   - `get_brand_identifier()`: 获取品牌缩写（用于文件命名）
...
3. **文件存储结构**
   - 按 "模型类型/品牌缩写_货号/" 结构组织
   - 图片命名：`品牌缩写_货号.{ext}`
   - 功能表命名：`品牌缩写_货号_FunctionKey.{ext}`
   - 说明书命名：`品牌缩写_货号_Manual_{原始文件名}.{ext}`
