...
from models import TrainsetSeries, TrainsetModel, LightBrand, LightModel
from models import LightModelCarriage, LightModelTrainset, LightModelBrandApplicability
...
def insert_reference_data():
  ...
  chip_model_defs = [
    ...
    ('动芯8004', ['PluX22']),
    ('动芯8003', ['MTC21']),
    ('ESU5.0', ['PluX22', 'NEXT18', 'MTC21']),
    ('MS450P22', ['PluX22']),
    ('XP5.1', ['PluX22']),
    ('Pragon4', ['NEXT18', 'PluX22']),
    ('Tsunami2', ['NEM652(8pin)']),
  ]
  for name, _interface_names in chip_model_defs:
    cm = ChipModel.query.filter_by(name=name).first()
    if not cm:
      cm = ChipModel(name=name)
      db.session.add(cm)
      db.session.flush()
    # 关联接口
    for iface_name in _interface_names:
      iface = ChipInterface.query.filter_by(name=iface_name).first()
      if iface and iface not in cm.interfaces:
        cm.interfaces.append(iface)
  ...
  # 13. 室内灯品牌、型号和适用车型（单一来源：utils/seeders/light_seed.py）
