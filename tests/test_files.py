...
from models import FunctionKey
...
def file_test_app():
  ...
  class FileTestConfig(TestConfig):
    ...
    ALLOWED_EXTENSIONS = {
      ...
      'function_table': {'pdf', 'doc', 'docx', 'xls', 'xlsx', 'jpg', 'jpeg', 'png', 'gif', 'webp'}
    }
...
class TestFunctionKeyAPI:
  """功能键 API 测试"""

  def test_get_keys_empty(self, file_test_app, file_test_client):
    """测试获取空的功能键列表"""
    with file_test_app.app_context():
      response = file_test_client.get('/api/files/function-keys/locomotive/1')
      assert response.status_code == 200
      data = response.get_json()
      assert data['success'] is True
      assert data['keys'] == []

  def test_get_keys_invalid_model_type(self, file_test_app, file_test_client):
    """测试无效的模型类型"""
    with file_test_app.app_context():
      response = file_test_client.get('/api/files/function-keys/locomotive_head/1')
      assert response.status_code == 400

  def test_update_keys_success(self, file_test_app, file_test_client):
    """测试更新功能键数据"""
    with file_test_app.app_context():
      keys_data = [
        {'key_number': 0, 'function_name': '头尾灯', 'description': ''},
        {'key_number': 1, 'function_name': '鸣笛', 'description': '短按短鸣'},
        {'key_number': 2, 'function_name': '制动', 'description': ''},
      ]
      response = file_test_client.put(
        '/api/files/function-keys/locomotive/1',
        data=json.dumps({'keys': keys_data}),
      ...
      assert data['keys'][0]['function_name'] == '头尾灯'

  def test_get_keys_after_update(self, file_test_app, file_test_client):
    """测试更新后获取功能键"""
    with file_test_app.app_context():
      # 先更新
      keys_data = [
      ...
      assert data['keys'][0]['key_number'] == 0
      assert data['keys'][0]['function_name'] == '头尾灯'

  def test_update_keys_overwrite(self, file_test_app, file_test_client):
    """测试更新功能键会覆盖旧数据"""
    with file_test_app.app_context():
      # 第一次写入
      keys_data1 = [
        {'key_number': 0, 'function_name': '头尾灯', 'description': ''},
        {'key_number': 1, 'function_name': '鸣笛', 'description': ''},
      ]
      file_test_client.put(
        '/api/files/function-keys/locomotive/1',
        data=json.dumps({'keys': keys_data1}),
        content_type='application/json'
      )

      # 第二次写入（覆盖）
      keys_data2 = [
        {'key_number': 0, 'function_name': '前照灯', 'description': '高亮'},
      ]
      response = file_test_client.put(
        '/api/files/function-keys/locomotive/1',
        data=json.dumps({'keys': keys_data2}),
        content_type='application/json'
      )
      data = response.get_json()
      assert data['success'] is True
      assert len(data['keys']) == 1
      assert data['keys'][0]['function_name'] == '前照灯'

  def test_update_keys_missing_data(self, file_test_app, file_test_client):
    """测试缺少 keys 数据"""
    with file_test_app.app_context():
      response = file_test_client.put(
        '/api/files/function-keys/locomotive/1',
  ...
  def test_reparse_no_file(self, file_test_app, file_test_client):
    """测试无功能表文件时重新解析"""
    with file_test_app.app_context():
      response = file_test_client.post('/api/files/reparse-function-table/locomotive/1')
      assert response.status_code == 404

  def test_export_keys_empty(self, file_test_app, file_test_client):
    """测试导出空的功能键"""
    with file_test_app.app_context():
      response = file_test_client.get('/api/files/function-keys/locomotive/1/export')
      assert response.status_code == 200
      assert 'spreadsheet' in response.content_type

  def test_export_keys_with_data(self, file_test_app, file_test_client):
    """测试导出有数据的功能键"""
    with file_test_app.app_context():
      # 先写入数据
      keys_data = [
