...
class TestChipOptions:
    ...
    def test_chip_model_add(self, client):
        ...
        # 先创建芯片接口
        with client.application.app_context():
            iface1 = ChipInterface(name='Next18')
            db.session.add(iface1)
            db.session.commit()
            iface1_id = iface1.id

        response = client.post('/options/chip_model', data={
            'name': '测试芯片型号',
            'interface_ids': [str(iface1_id)]
        }, follow_redirects=True)
        assert response.status_code == 200
        ...
        with client.application.app_context():
            ...
            assert len(cm.interfaces) == 1
            assert cm.interfaces[0].id == iface1_id

    def test_chip_model_add_multiple_interfaces(self, client):
        """测试添加芯片型号（多接口）"""
        with client.application.app_context():
            iface1 = ChipInterface(name='Next18')
            iface2 = ChipInterface(name='PluX22')
            db.session.add(iface1)
            db.session.add(iface2)
            db.session.commit()
            iface1_id = iface1.id
            iface2_id = iface2.id
        ...
        response = client.post('/options/chip_model', data={
            'name': '多接口芯片',
            'interface_ids': [str(iface1_id), str(iface2_id)]
        }, follow_redirects=True)
        assert response.status_code == 200

        with client.application.app_context():
            cm = ChipModel.query.filter_by(name='多接口芯片').first()
            assert cm is not None
            assert len(cm.interfaces) == 2
